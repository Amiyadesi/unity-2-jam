...
##
## 注意：这里会自动调用 `DialogueLabel.bbcode_to_plain_text()` 清理 BBCode 与特效标签，
## 这样存档摘要不会把 `[sparkle]...[]`、`[sin]...[]` 之类的渲染标签直接存进 UI。
func save_progress(
	...
	dialogue_snippet       = DialogueLabel.bbcode_to_plain_text(snippet).left(60)  # 最多保存 60 个字符的摘要
