...
func _connect_ui() -> void:
	...
	if not _responses_menu.response_selected.is_connected(_on_response_selected):
...
func _show_responses_for_line(line: DialogueLine) -> void:
	if line.responses.is_empty():
		_responses_menu.hide()
		return
	if is_instance_valid(_dialogue_label) and not line.text.is_empty():
		await _dialogue_label.finished_typing
	if get_current_line() != line:
		return
	_responses_menu.responses = line.responses
	_responses_menu.show()
	_responses_menu.configure_focus()


func _on_response_selected(response: DialogueResponse) -> void:
	next(response.next_id)
	emit_module_event("response_selected", {"response": response})


func _sync_status_indicators() -> void:
	if is_instance_valid(_auto_advance_indicator):
		_auto_advance_indicator.text = "⟳ 自动" if _flow_module.auto_advance else ""
		_auto_advance_indicator.visible = _flow_module.auto_advance
	if not is_instance_valid(_speed_indicator):
		return
	if _flow_module.current_speed_multiplier > 1.5:
		_speed_indicator.text = "⏩ 快进 x%.1f" % _flow_module.current_speed_multiplier
		_speed_indicator.show()
	elif _flow_module.current_speed_multiplier < 0.7:
		_speed_indicator.text = "⏪ 慢放 x%.1f" % _flow_module.current_speed_multiplier
		_speed_indicator.show()
	else:
		_speed_indicator.hide()
