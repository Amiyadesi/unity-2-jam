...
func _update_direction(character: String) -> void:
	...
	var portrait_container := portrait_texture.get_parent() if is_instance_valid(portrait_texture) else null
	var content_container := character_label.get_parent() if is_instance_valid(character_label) else null
	if not (portrait_container is Control) or not (content_container is Control):
		return
	var h_box := portrait_container.get_parent() as HBoxContainer
	if h_box == null:
		return
	match target_direction:
		"right":
			if portrait_container.get_index() != h_box.get_child_count() - 1:
				h_box.move_child(portrait_container, h_box.get_child_count() - 1)
		_:
