...
func _on_typing_finished(line: DialogueLine) -> void:
	...
	if auto_advance:
		...
		if auto_advance_mode == "text_length":
			_auto_advance_timer = max(auto_advance_delay, _get_visible_text_length(line) * auto_advance_text_multiplier)
		else:
...
func _handle_voice_tag(line: DialogueLine) -> void:
	...
	if is_instance_valid(voice_player):
		...
		if not voice_path.is_empty():
			...
			voice_player.play()
			while is_instance_valid(voice_player) and voice_player.playing and _current_line == line and line_serial == _line_serial:
				await get_tree().process_frame
	if is_instance_valid(_balloon) and _current_line == line and line_serial == _line_serial:
...
func _handle_time_tag(line: DialogueLine) -> void:
	var time: float
	if line.time == "auto":
		time = _get_visible_text_length(line) * base_typing_speed
	else:
...
func _get_visible_text_length(line: DialogueLine) -> int:
	if is_instance_valid(dialogue_label) and dialogue_label.dialogue_line == line:
		return dialogue_label.get_display_character_count()
	return DialogueLabel.bbcode_to_plain_text(line.text).length()
