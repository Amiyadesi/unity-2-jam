...
var _window_transition_layers := {}
var _window_transition_items := {}
var _window_transition_tweens := {}
var _window_transition_durations := {}

func transition_clear(target_window: Window = null) -> void:
	var canvas_item := _get_transition_canvas_item(target_window)
	canvas_item.hide()
	_set_transition_duration_for_window(target_window, 0.0)
...
func transition_set_duration(duration: float, target_window: Window = null) -> void:
	_set_transition_duration_for_window(target_window, duration)

##   Starts a transition. Returns a tween whose finished signal indicates the
## transition is finished.
func transition_start(transition: SceneTransition, reverse := false, target_window: Window = null) -> Tween:
	var old_tween := _get_transition_tween_for_window(target_window)
	...
	var duration := _get_transition_duration_for_window(target_window)
	var tween := create_tween()
	tween.tween_method(func(shader_value: float) -> void:
		_transition_set_shader_completion_for_item(canvas_item, shader_value),
		value,
		1.0 - value,
		transition.duration if duration == 0.0 else duration
	)
	_set_transition_tween_for_window(target_window, tween)

	return tween

## Sets shader completion parameter.
func _transition_set_shader_completion(value: float) -> void:
	_transition_set_shader_completion_for_item(_canvas_item, value)

func _transition_set_shader_completion_for_item(canvas_item: CanvasItem, value: float) -> void:
	var material := canvas_item.material as ShaderMaterial
	if material != null:
		material.set_shader_parameter("completion", value)

func _get_transition_canvas_item(target_window: Window = null) -> CanvasItem:
	if target_window == null:
		return _canvas_item

	var key := target_window.get_instance_id()
	var existing := _window_transition_items.get(key) as CanvasItem
	if existing != null and is_instance_valid(existing):
		return existing

	var layer_node := CanvasLayer.new()
	layer_node.name = "SceneManagerTransitionLayer"
	layer_node.layer = layer

	var color_rect := ColorRect.new()
	color_rect.name = "CanvasItem"
	color_rect.mouse_filter = Control.MOUSE_FILTER_IGNORE
	color_rect.color = Color.WHITE
	color_rect.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	if _canvas_item.material != null:
		color_rect.material = _canvas_item.material.duplicate(true)
	color_rect.hide()

	target_window.add_child(layer_node)
	layer_node.add_child(color_rect)
	_window_transition_layers[key] = layer_node
	_window_transition_items[key] = color_rect
	target_window.tree_exiting.connect(_on_transition_window_exiting.bind(key), CONNECT_ONE_SHOT)
	return color_rect

func _on_transition_window_exiting(key: int) -> void:
	var tween := _window_transition_tweens.get(key) as Tween
	if tween != null and tween.is_valid():
		tween.kill()
	_window_transition_layers.erase(key)
	_window_transition_items.erase(key)
	_window_transition_tweens.erase(key)
	_window_transition_durations.erase(key)

func _get_transition_tween_for_window(target_window: Window = null) -> Tween:
	if target_window == null:
		return _transition_tween
	return _window_transition_tweens.get(target_window.get_instance_id()) as Tween

func _set_transition_tween_for_window(target_window: Window = null, tween: Tween = null) -> void:
	if target_window == null:
		_transition_tween = tween
		return
	_window_transition_tweens[target_window.get_instance_id()] = tween

func _get_transition_duration_for_window(target_window: Window = null) -> float:
	if target_window == null:
		return _transition_duration
	return float(_window_transition_durations.get(target_window.get_instance_id(), 0.0))

func _set_transition_duration_for_window(target_window: Window = null, duration: float = 0.0) -> void:
	if target_window == null:
		_transition_duration = duration
		return
	_window_transition_durations[target_window.get_instance_id()] = duration
#endregion
