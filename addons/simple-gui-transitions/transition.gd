...
func _get_gui_transitions_singleton() -> Node:
	if get_tree() == null or get_tree().root == null:
		return null
	return get_tree().root.get_node_or_null("GuiTransitions")
...
func _ready() -> void:
	...
	if _transition_valid():
		...
		var gui_transitions := _get_gui_transitions_singleton()
		if gui_transitions != null:
			var layouts := gui_transitions.get("_layouts") as Dictionary
			if not layouts.has(layout_id):
				layouts[layout_id] = []
			(layouts[layout_id] as Array).push_back(self)
			gui_transitions.set("_layouts", layouts)
...
func _exit_tree() -> void:
	...
	if gui_transitions == null:
		return
	var layouts_dict := gui_transitions.get("_layouts") as Dictionary
	var layouts: Array = layouts_dict.get(layout_id, [])
	...
	if not layouts.size():
		layouts_dict.erase(layout_id)
	else:
		layouts_dict[layout_id] = layouts
	gui_transitions.set("_layouts", layouts_dict)
...
func _go_to(id := "", function = null):
	...
	if _transition_valid() and _layout.visible:
		...
		if id != layout_id:
			...
			gui_transitions.call("_for_each_layout", "_show", [id])
		else:
			gui_transitions.call("_for_each_layout", "_show", [id])
...
func _show(id := ""):
	if _transition_valid() and (not id or id == layout_id) and _status == Status.OK:
		...
		var gui_transitions := _get_gui_transitions_singleton()
		if gui_transitions != null and bool(gui_transitions.call("is_shown", layout_id)):
			gui_transitions.emit_signal("show_completed")
...
func _hide(id := "", function = null):
	if _transition_valid() and _layout.visible and (not id or id == layout_id) and _status == Status.OK:
		...
		var gui_transitions := _get_gui_transitions_singleton()
		if gui_transitions != null and bool(gui_transitions.call("is_hidden", layout_id)):
			gui_transitions.emit_signal("hide_completed")
